...
const { validateAccessToken, findSchoolID_query, findSchoolID_params, findSchoolID_body, findDistrictID_querySearchParams, checkEDXUserAccessToRequestedInstitute, checkEdxUserPermission, verifyQueryParamValueMatchesBodyValue, forbidActionOnOffshoreSchools } = require('../components/permissionUtils');
...
router.get('/allSchools', passport.authenticate('jwt', {session: false}, undefined), isValidBackendToken, validateAccessToken, validate(schoolPaginatedSchema), findDistrictID_querySearchParams, checkEDXUserAccessToRequestedInstitute, getAllSchoolDetails);
router.get('/schoolDetailsById/:schoolID', passport.authenticate('jwt', {session: false}, undefined), isValidBackendToken, validateAccessToken, validate(getSchoolSchema), findSchoolID_params, checkEDXUserAccessToRequestedInstitute, getFullSchoolDetails);
router.put('/:schoolID', passport.authenticate('jwt', {session: false}, undefined), isValidBackendToken, validateAccessToken, validate(putSchoolSchema), verifyQueryParamValueMatchesBodyValue('schoolID', 'schoolId'), checkEdxUserPermission(PERMISSION.EDX_SCHOOL_EDIT), findSchoolID_params, checkEDXUserAccessToRequestedInstitute, forbidActionOnOffshoreSchools, updateSchool);
router.post('/:schoolID/contact', passport.authenticate('jwt', {session: false}, undefined), isValidBackendToken, validateAccessToken, validate(schoolContactSchema), checkEdxUserPermission(PERMISSION.EDX_SCHOOL_EDIT), findSchoolID_params, checkEDXUserAccessToRequestedInstitute, forbidActionOnOffshoreSchools, addSchoolContact);
