...
function createEllYearsFilter(pValue) {
  let ellList = [];
  const range = pValue[0];
  if (range) {
    ellList.push({ key: 'yearsInEll', value: range[0], operation: FILTER_OPERATION.GREATER_THAN_OR_EQUAL_TO, valueType: VALUE_TYPE.INTEGER, condition: CONDITION.AND });
    ellList.push({ key: 'yearsInEll', value: range[1], operation: FILTER_OPERATION.LESS_THAN_OR_EQUAL_TO, valueType: VALUE_TYPE.INTEGER, condition: CONDITION.AND });
  }
  return ellList;
}
