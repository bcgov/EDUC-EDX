<template>
    ...
    >
      ...
      <v-card-text>
        ...
        <v-row>
            <v-col cols="6">
                <StudentDetail :student="selectedProgramDuplicate?.sdcSchoolCollectionStudent1Entity" :duplicate-type-code="selectedProgramDuplicate?.programDuplicateTypeCode"></StudentDetail>
            </v-col>           
            <v-col >
                <StudentDetail :student="selectedProgramDuplicate?.sdcSchoolCollectionStudent2Entity" :duplicate-type-code="selectedProgramDuplicate?.programDuplicateTypeCode"></StudentDetail>
            </v-col>
        </v-row>

         <v-row class="mt-6">
          <v-col class="label">Duplicate Programs:</v-col>
        </v-row>
    ...
    <v-row>
          <v-col >
            ...
            <div v-for="(prog, index) in getDuplicatePrograms()">
    <span>{{ prog?.description }}</span>

      <v-radio-group v-model="selected[index]" direction="horizontal"
                    :inline="true" :rules="[v => !!v || 'Required']" validate-on="input" required>
        <v-radio :id="`index`" :label="prog?.studentOne?.schoolName" :value="{dupeCode: prog?.code, studentId: prog?.studentOne?.sdcSchoolCollectionStudentID}"></v-radio>
        <v-radio :id="`index`" :label="prog?.studentTwo?.schoolName" :value="{dupeCode: prog?.code , studentId: prog?.studentTwo?.sdcSchoolCollectionStudentID}"></v-radio>
      </v-radio-group>
  </div>
          </v-col>
